...
function msteams_plugin_row_meta( $links, $file ) {
    ...
    if ( MSTEAMS_TEXTDOMAIN.'/'.MSTEAMS_TEXTDOMAIN.'.php' == $file ) {
        ...
        // Add the link
        $row_meta = [
            // 'docs' => '<a href="'.esc_url( 'https://apos37.com/' ).'" target="_blank" aria-label="'.esc_attr__( 'Plugin Website Link', 'gf-msteams' ).'">'.esc_html__( 'Website', 'gf-msteams' ).'</a>',
            'discord' => '<a href="'.esc_url( 'https://discord.gg/VeMTXRVkm5' ).'" target="_blank" aria-label="'.esc_attr__( 'Plugin Support on Discord', 'gf-msteams' ).'">'.esc_html__( 'Discord Support', 'gf-msteams' ).'</a>'
        ];
